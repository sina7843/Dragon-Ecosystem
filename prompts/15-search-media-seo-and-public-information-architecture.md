# DRAGON-15 — Search, media, SEO, and public information architecture
...
Mission:
Complete public discoverability through safe media handling, search/filtering, SEO, sitemap, metadata, and localized routing.
...
Acceptance criteria:
- Implement approved media upload/staging/publication with signature validation, size/type limits, safe names, authorization, and storage adapter boundaries.
- Implement paginated search and filters for content, games, tournaments, and permitted profiles/teams without leaking drafts or restricted data.
- Implement localized metadata, canonical/hreflang, robots, XML sitemap, structured data where applicable, redirects, and real HTTP status behavior.
- Ensure crawlable production output exposes meaningful public content and disabled future modules do not inflate launch bundles.
- Add security, authorization, SEO, direct-refresh, localization, and representative browser tests.
