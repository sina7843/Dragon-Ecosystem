...
Mission:
Build the modular-monolith shared kernel, API contract conventions, Mongo access layer, migrations, seeds, jobs, and test foundations.
...
Acceptance criteria:
- Define domain-module boundaries, dependency rules, shared error format, request correlation, runtime validation, OpenAPI generation, and versioned `/api/v1` routing.
- Implement Mongo connection management, migration/version tracking, idempotent development seed, transaction/session helpers, index declarations, and test database isolation.
- Implement shared identity IDs, audit-event envelope, outbox/job interfaces, money value objects using integer rial, Dragon Coin integer units, and UTC timestamp conventions.
- Add API contract, integration, migration, index, idempotency, and restart-persistence tests.
- No domain feature may bypass validation, authorization context, audit correlation, or repository boundaries.
