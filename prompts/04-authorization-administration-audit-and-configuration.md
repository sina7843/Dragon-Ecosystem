# DRAGON-04 — Authorization, administration, audit, and configuration
...
Mission:
Implement server-enforced RBAC/resource scope, administrative shell, immutable audit evidence, and versioned system configuration.
...
Acceptance criteria:
- Implement roles and permissions required for super admin, content, tournament, finance, moderation, support, and ordinary users, with deny-by-default authorization.
- Implement admin user/role management, suspension/reactivation, configuration versions, reason requirements, dual-control hooks for high-risk finance actions, and emergency-action review.
- Record immutable audit events for every high-risk mutation with actor, scope, reason, before/after references, correlation, and timestamp.
- Add authorization-matrix tests proving 401/403/404 behavior and preventing IDOR and privilege escalation.
- Expose audit search/export only to authorized roles with pagination and redaction.
