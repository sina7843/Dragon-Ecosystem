...
$required = @(
    ...
    'PROJECT_STATUS.md',
    'prompts\00-foundation-bootstrap-and-governance.md',
...
try {
    $mongoListener = @(Get-NetTCPConnection -LocalPort 27017 -State Listen -ErrorAction SilentlyContinue)
    if ($mongoListener.Count -gt 0) {
        Write-Warning 'Host port 27017 is already occupied. DRAGON-00 is configured not to publish MongoDB on the host, so Compose should use only the internal mongo:27017 service address.'
    }
} catch {}
...
if ($commitCount -eq 0) {
    ...
    if ([string]::IsNullOrWhiteSpace($localName)) {
        $safeName = if ($env:USERNAME) { $env:USERNAME } else { 'Dragon Developer' }
    ...
    if ([string]::IsNullOrWhiteSpace($localEmail)) {
        $safeUser = if ($env:USERNAME) { $env:USERNAME.ToLowerInvariant() } else { 'dragon-developer' }
    ...
    Run-Git @('commit', '-m', 'chore: baseline Dragon ecosystem starter') 'Baseline commit failed. Do not run DRAGON-00 until this succeeds.'

    $backupDir = Join-Path $Root '.dragon-backups'
    ...
    if (-not [string]::IsNullOrWhiteSpace($dirty)) {
        Write-Warning 'The repository already has commits but also has uncommitted changes. Commit or stash them before DRAGON-00.'
    }
}
...
if ([string]::IsNullOrWhiteSpace($finalCommitCount) -or [int]$finalCommitCount.Trim() -lt 1) {
    throw 'No recoverable Git commit exists. DRAGON-00 remains blocked.'
}
...
Write-Host 'Next: run 03-CHECK-PACKAGE.cmd. Then copy DRAGON-00 with 04-COPY-NEXT-PROMPT.cmd and start Claude with 05-START-CLAUDE.cmd.'
