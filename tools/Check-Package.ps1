...
if ([string]::IsNullOrWhiteSpace($commitCountText) -or [int]$commitCountText.Trim() -lt 1) {
    throw 'No baseline commit exists. Run 02-SETUP-PROJECT.cmd and do not run DRAGON-00 yet.'
}
if (-not (Test-Path '.dragon-backups\baseline.bundle')) {
    Write-Warning 'The local baseline Git bundle is missing. Re-run setup or create a private remote before DRAGON-00.'
}
