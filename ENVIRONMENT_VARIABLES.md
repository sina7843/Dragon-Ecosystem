...
| `MONGODB_URI` | MongoDB connection string. | all; **mandatory** in production | No today; becomes secret once credentials are added | `mongodb://…` or `mongodb+srv://…` | `mongodb://mongo:27017/dragon` outside production only | Rotate with the database credential when authentication is enabled | api |
