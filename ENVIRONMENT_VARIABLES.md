...
| `PUBLIC_ORIGIN` | Absolute public origin the API uses to build `sitemap.xml`, `robots.txt`, and canonical URLs (SEO-005/006). Empty yields site-relative locations (fine locally); set to the real `https://` origin in production so crawlers get absolute URLs. Trailing slashes are trimmed. | all | No | absolute origin, e.g. `https://dragon.example` | empty | Not applicable | seo |
