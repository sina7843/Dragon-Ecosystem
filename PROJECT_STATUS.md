...
**Product scope restored to the DRAGON-17 / Phase-One capability set** (baseline commit `c32967d`) by the DRAGON-R1 controlled rollback.

**Post-Phase-One capabilities are temporarily removed pending revised product specifications.** They are removed from the source tree — not disabled — so no route, page, API endpoint, permission, feature gate, domain event, notification template, migration, or environment variable for them remains. The later-phase items below are unchecked for that reason, exactly as they were at DRAGON-17.

Removed: streams (DRAGON-18), live chat (DRAGON-19), education/courses (DRAGON-20), community/social and delegated team roles (DRAGON-22), store and orders (DRAGON-24), economy transfers/rewards/payout hardening (DRAGON-25), match scheduling (DRAGON-29D), participant registration history (DRAGON-29E), the public help surface (DRAGON-29F), and the generated site guides (DRAGON-30).

Independent post-DRAGON-17 fixes were retained where they still protect Phase-One code — see `DECISIONS.md` (2026-07-31).
