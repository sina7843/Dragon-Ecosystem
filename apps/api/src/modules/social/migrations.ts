import type { Db } from 'mongodb';
import type { Migration } from '../../shared/db/migrations.ts';
import { SOCIAL_COLLECTIONS, SOCIAL_INDEXES } from './collections.ts';

/** Creates the social graph, post, comment, reaction, and profile collections. Idempotent. */
export const socialMigration: Migration = {
  version: '027-social',
  description: 'Create the social follow, post, comment, reaction, and profile collections.',

  async up(db: Db): Promise<void> {
    const existing = new Set((await db.listCollections({}, { nameOnly: true }).toArray()).map((c) => c.name));
    for (const name of Object.values(SOCIAL_COLLECTIONS)) {
      if (!existing.has(name)) await db.createCollection(name);
    }
    for (const declaration of SOCIAL_INDEXES) {
      await db.collection(declaration.collection).createIndex(declaration.keys, { name: declaration.name, ...declaration.options });
    }
  }
};
